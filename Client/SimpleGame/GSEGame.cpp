		...
		for (int j = 0; j < MAP_SIZE; j++)
		{
			...
			if (m_pMapdata[i][j].playerColor != PlayerColor::PLAYEREMPTY)
			{
				...
				case PlayerColor::RED:
					m_renderer->DrawSolidRect(i * 20, j * 20, 0, 20, 1.0f, 0.0f, 0.0f, 1.0f);	//빨강
					break;
				case PlayerColor::GREEN:
					m_renderer->DrawSolidRect(i * 20, j * 20, 0, 20, 0.0f, 1.0f, 0.0f, 1.0f);	//초록
					break;
				case PlayerColor::BLUE:
					m_renderer->DrawSolidRect(i * 20, j * 20, 0, 20, 0.0f, 0.0f, 1.0f, 1.0f);	//파랑
					break;
				}
			}
		}
